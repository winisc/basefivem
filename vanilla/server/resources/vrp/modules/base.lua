...
AddEventHandler("SaveServer", function()
    for k, v in pairs(Sources) do
        ...
        if Datatable then
            vRP.Query("playerdata/SetData", { Passport = k, Name = "Datatable", Information = json.encode(Datatable) })
        end
    end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
...
function vRP.CharacterChosen(source, Passport, Model)
    ...
    if not Characters[source] then
        ...
        exports["vrp"]:Embed("Connect","**Source:** " .. source .. "\n**Passaporte:** " .. Passport .. "\n**Ip:** " .. GetPlayerEndpoint(source),0xa3c846)
        exports["vrp"]:Embed("Login","**Discord:** " .. Account["discord"] .. "\n**Passaporte:** " .. Passport .. "\n**Name:** ".. Query[1]["name"] .. "\n**Lastname:** " .. Query[1]["lastname"],0xa3c846)
    end
