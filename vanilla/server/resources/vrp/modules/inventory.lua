...
function vRP.CheckDamaged(Item)
    if itemDurability(Item) and splitString(Item, "-")[2] and
        parseInt((86400 * itemDurability(Item) - parseInt(os.time() - splitString(Item, "-")[2])) / (86400 * itemDurability(Item)) * 100) <= 1 then
        return true
    end
    return false
end
...
function vRP.GiveItem(Passport, Item, Amount, Notify, Slot)
    ...
    if source and parseInt(Amount) > 0 then
        ...
        if not Slot then
            ...
            repeat
                newSlot = newSlot + 1
            until Inventory[tostring(newSlot)] == nil or Inventory[tostring(newSlot)] and
                Inventory[tostring(newSlot)]["item"] == Item
...
function vRP.GetSrvData(Key)
    if DataServer[Key] == nil then
        local rows = vRP.Query("entitydata/GetData", { Name = Key })
        if parseInt(#rows) > 0 then
            DataServer[Key] = { data = json.decode(rows[1]["Information"]), timer = 180 }
        else
            DataServer[Key] = { data = {}, timer = 180 }
        end
    end

    return DataServer[Key]["data"]
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- SETSRVDATA
-----------------------------------------------------------------------------------------------------------------------------------------
function vRP.SetSrvData(Key, Data)
    DataServer[Key] = { data = Data, timer = 180 }
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- REMSRVDATA
-----------------------------------------------------------------------------------------------------------------------------------------
...
CreateThread(function()
    while true do
        for k, v in pairs(DataServer) do
            if v["timer"] > 0 then
                v["timer"] = v["timer"] - 1
...
function vRP.TakeChest(Passport, Data, Amount, Slot, Target)
    ...
    if source and parseInt(Amount) > 0 and Datatable[Slot] then
        ...
        if vRP.MaxItens(Passport, Datatable[Slot]["item"], (parseInt(Amount))) then
            TriggerClientEvent("Notify", source, "amarelo", "Limite atingido.","Atenção",3000)
            return true
        end
        if vRP.InventoryWeight(Passport) + itemWeight(Datatable[Slot]["item"]) * parseInt(Amount) <=
            vRP.GetWeight(Passport) then
            if Inventory[Target] then
                if Datatable[Slot] and Inventory[Target]["item"] == Datatable[Slot]["item"] and parseInt(Amount) <=
                    Datatable[Slot]["amount"] then
...
function vRP.StoreChest(Passport, Data, Amount, Weight, Slot, Target)
    ...
    Slot = tostring(Slot)
    Target = tostring(Target)
    if source and parseInt(Amount) > 0 and Inventory[Slot] and Weight >= vRP.ChestWeight(Datatable) +
        itemWeight(Inventory[Slot]["item"]) * parseInt(Amount) then
        if Datatable[Target] then
            if Inventory[Slot] then
                ...
                if 0 >= Inventory[Slot]["amount"] then
                    if "Armamento" == itemType(Inventory[Slot]["item"]) or "Throwing" ~=
                        itemType(Inventory[Slot]["item"]) then
            ...
            if Inventory[Target] then
                if Inventory[Slot]["item"] == Inventory[Target]["item"] then
                    ...
                    if Inventory[Slot]["item"] and Inventory[Target]["item"] then
                        if Inventory[Slot]["item"] == "dollars" and "suitcase" ==
                            splitString(Inventory[Target]["item"], "-")[1] then
                        ...
                        elseif Inventory[Slot]["item"] == "repairkit0" .. string.sub(Inventory[Slot]["item"], 11, 12) then
                            if vRP.CheckDamaged(Inventory[Target]["item"]) and 1 == Inventory[Target]["amount"] then
                                if Inventory[Slot]["item"] == "repairkit0" ..
                                    string.sub(Inventory[Slot]["item"], 11, 12) then
                                    if itemRepair(Inventory[Target]["item"]) then
                                        if itemRepair(Inventory[Target]["item"]) == Inventory[Slot]["item"] then
                                            ...
                                            TriggerClientEvent("Notify", source, "amarelo","Seu item pode ser reparado com <b>" ..itemName((itemRepair(Inventory[Target]["item"]))) .. "</b>.","Atenção", 5000)
                                        end
                                    end
                                end
                            end
